...
export class BattleScene {
  ...
  renderSeqBox(ctx, x, y, w, h) {
    ...
    // 连招提示：接「轻」「重」各自会触发哪一招（这一按成不了招的那一边就不列）
    let hy = y + 94;
    for (const h of this.comboHints()) {
      text(ctx, `${h.label}→${h.skill.name}`, x, hy, {
        size: 11, color: h.ok ? PAL.jadeHi : '#8a6a2f',
      });
      hy += 15;
    }
  }
  ...
  /**
   * 连招提示：接「轻」「重」各自会触发哪一招。
   * 只有「这一按确实成招」——序列后缀匹配到招式、且玩家已学会——才列出；
   * 比如已有两个轻时，接轻是风卷残云、接重是白虹贯日，接不出的那一边就不显示。
   * 内力不足时照常列出、只把颜色压暗，免得玩家以为这串序列没有了后续。
   */
  comboHints() {
    const p = this.p;
    const out = [];
    for (const [moveId, label] of [['light', '轻'], ['heavy', '重']]) {
      const sk = matchSkill(p.seq.concat([seqStep(moveId)]));
      if (!sk || !this.P.hasSkill(sk.id)) continue;
      out.push({ label, skill: sk, ok: p.qi >= sk.qi });
    }
    return out;
  }
  ...
  renderTrack(ctx, x, y, w, h) {
    ...
    if (awaiting) {
      const spd = this.P.speedMul;
      // 轻／重两候选：这一按若会**触发连招**（序列后缀成招 + 已学会 + 内力够），
      // 落点用连招的 cd，而不是基础招式的固定 cd——否则箭头指的时刻跟按下后时间轴
      // 真正会走到哪一步对不上，读轴就读错了。标签仍只写「轻」「重」，不写招式名。
      const moveCand = (moveId, label) => {
        const mv = BASE_MOVES[moveId];
        const sk = matchSkill(p.seq.concat([seqStep(moveId)]));
        const cast = sk && this.P.hasSkill(sk.id) && p.qi >= sk.qi;
        return {
          name: label,
          axis: cast ? sk.cd : mv.cd,
          ok: cast ? true : p.stam >= mv.stamina,
          tint: cast ? sk.tint : mv.tint,
          row: 0,
        };
      };
      // 候选分两排：防御（闪避／格挡）在上、攻击（轻／重）在下。
      // 同排两项的落点天然隔得开（闪避 0.30 vs 格挡 0.50、轻 0.36 vs 重 0.60），
      // 两排正好把四个标签装下；若因轴段、缩放或连招落点挤住，会顺延到下一排兜底，不至于叠字
      const items = [
        moveCand('light', '轻'),
        moveCand('heavy', '重'),
        { name: '闪避', axis: EVADE.axis, ok: p.stam >= EVADE.stamina, tint: EVADE.tint, row: 1 },
        { name: '格挡', axis: GUARD.axis, ok: p.stam >= GUARD.stamina, tint: GUARD.tint, row: 1 },
