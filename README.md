...
| 战斗 | `U` | 绝学「无明剑意」，**剑意满**时可放（表演 0.75s + 轴段 0.30s，与闪避同长），放后剑意清零；剑意只在秘境中积攒 |
...
轻／重若这一按会**搓成连招**，落点就是**连招的 cd**，
不会还停在基础招式的固定位置上——箭头指着哪一刻，按下去时间就走到哪一刻
（标签上仍只写「轻」「重」，具体接到什么招看连招框下方的提示）。

四个候选的标签一般分两排：**防御在上（闪避／格挡）、攻击在下（轻／重）**，
挤住时自动顺延到下一排，不叠字：
...
框的下面是**连招提示**：接着按「轻」「重」分别会出哪一招——已有两个轻时就是
「轻→风卷残云」「重→白虹贯日」；**搓不出招的那一边不列**（内力不够时照列，只是压暗）。
